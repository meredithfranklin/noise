...
try:
    ...
    df_pre = pd.read_csv(r"/Codes/sound_pollutant_met_data.csv", nrows=253605)
    print(df_pre.head())
...
# to replace the space within each column name string into _
# 'time utc' -> 'time_utc'

# Step 3: Convert time_utc to datetime
df_pre['time_utc'] = pd.to_datetime(df_pre['time_utc'], errors='coerce')
# pd.to_datetime() tries to convert values into a proper datetime64[ns] object.
# This is critical to filter or do time-based operations
# (e.g. sorting, resampling, filtering by date).
# If a value in time_utc can’t be converted to a valid datetime,
# it will be replaced with NaT (errors = 'coerce')
...
### for pandas data file, it is very powerful.
# Not only pd['column_name'] can select the specific column from data
# df[(logical operations)] can also perform logical operations to select specific
# range of values in the data for specific columns.
...
## this creates an upper triangular matrix of True that will mask the heatplot
## (aka. the heatplot will not show the upper triangular elements.)
...
## This creates a matrix of boolean (True & False) that tells
# the pyplot to plot which elements and to hide which elements (mask.)
# '|' means or, '&' means and.
# the third condition ((corr_matrix == 1.0)) is redundant
# because the upper triangular already include the diagonal elements.
...
def calculate_vif(X):
    ...
    """
    vif_data = pd.DataFrame()  ## generate a new pd data
...
for response in ['nox', 'ch4', 'h2s', 'co', 'co2_ppm']:
    ...
    while vif_data["VIF"].max() > 20:
        ...
        # iloc[] is used to access rows by their integer position
        # we select the first row after sorted the vif to be descending
        # (first row is the largest VIF because of descending order)
        # In VIF function, each row represents a predictor variable
        # and its corresponding VIF score.
